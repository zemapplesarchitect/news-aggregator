...
- **dashboard.py** — Reads metrics JSON files, computes 7-day and 30-day summaries, renders a markdown table, and injects it into `README.md` between `<!-- DASHBOARD:START/END -->` markers. Cost column aggregates per-run stored costs (not recalculated). Runnable standalone: `uv run python -m src.dashboard`
